...
class ManagerLoginPopup extends Component {
    constructor(props){
        super(props); //props should be closePopup and action handler
        this.state = {
            companyID: 0,
            employeeID: 0,
            showPopup: false,
  ...
  handleSubmit = (event) => {
    ...
    if (form.companyId.value.trim() !== "1" && form.companyId.value.trim() !== "2") {
      errorLabel.className = "invalid";
      errorLabel.innerHTML = "Please enter a valid Company ID";
      valid = false;
    }
    //once valid, proceed
    if (valid) {
